/**
 * Term Ordering Script.
 *
 * Enables drag-and-drop sorting for collection taxonomies in the WordPress admin.
 * Based on WooCommerce's term-ordering.js.
 *
 * @package IELTS_Science_LMS
 */

/*global ajaxurl, ieltssci_term_ordering_params */
...
jQuery(function ($) {
	...
	/**
	 * Add missing sort handles after AJAX operations.
	 */
	$.ieltssci_add_missing_sort_handles = function () {
	...
	$(document).ajaxComplete(function (event, request, options) {
		if (
			...
			$.ieltssci_add_missing_sort_handles();
		}
	});
	...
	$(table_selector).sortable({
		...
		update: function (event, ui) {
			var termid = ui.item.find(term_id_selector).val();
			...
			$.post(
				ajaxurl,
				{
					action: "ieltssci_term_ordering",
					id: termid,
					nextid: nexttermid,
					thetaxonomy: ieltssci_term_ordering_params.taxonomy,
					security: ieltssci_term_ordering_params.nonce,
				},
				function (response) {
