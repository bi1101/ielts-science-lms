<?php
/**
 * IELTS Science Reading Module
 *
 * This file contains the implementation of the Reading Module for IELTS Science LMS.
 *
 * @package IeltsScienceLMS\Reading
 */

namespace IeltsScienceLMS\Reading;

use WP_REST_Users_Controller;
use WP_REST_Request;

/**
 * Class Ieltssci_Reading_Module
 *
 * Handles the functionality for the IELTS Science Reading Module.
 * Manages assets, routes, and data for the reading module features.
 */
class Ieltssci_Reading_Module {
	/**
	 * Constructor for the Ieltssci_Reading_Module class.
	 *
	 * Initializes the reading module by setting up hooks and loading dependencies.
	 */
	public function __construct() {
		// Placeholder for reading-specific classes.
		// new Ieltssci_Reading_SSE_REST();
		// new Ieltssci_Reading_Practice_Controller();
		// new Ieltssci_Reading_Settings();
		// new Ieltssci_Reading_Entries();
		// new Ieltssci_Submission_DB();
		// new Ieltssci_Reading_Task_Submission_Controller();
		// new Ieltssci_Reading_Test_Submission_Controller();
		// new Ieltssci_Reading_Vote_Controller();.

		// Initialize the reading module.
		add_action( 'wp_enqueue_scripts', array( $this, 'register_reading_assets' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_reading_assets' ), 100 );
	...
	/**
	 * Register assets (scripts and styles) for the reading module.
	 *
	 * This function locates and registers JavaScript and CSS files required for the reading module.
	 * Asset files are expected to be in the 'public/reading/build/' directory.
	 *
	 * @return void
	 */
	public function register_reading_assets() {
		$build_path  = plugin_dir_path( __FILE__ ) . '../../public/reading/build/';
		$asset_files = glob( $build_path . '*.asset.php' );

		foreach ( $asset_files as $asset_file ) {
			$asset  = include $asset_file;
			$handle = 'ielts-science-reading-' . basename( $asset_file, '.asset.php' );
			$src    = plugin_dir_url( __FILE__ ) . '../../public/reading/build/' . basename( $asset_file, '.asset.php' ) . '.js';
			...
			if ( file_exists( $css_file ) ) {
				...
				$css_src    = plugin_dir_url( __FILE__ ) . '../../public/reading/build/' . basename( $css_file );
	...
	/**
	 * Enqueue assets for the reading module based on the current page.
	 *
	 * Loads necessary scripts and styles when the user is on a reading module page.
	 * Provides localized data to the JavaScript including page routes, user info, and menus.
	 *
	 * @return void
	 */
	public function enqueue_reading_assets() {
		...
		// Extract reading module pages.
		$reading_module_pages = array();
		if ( isset( $module_pages_data['reading_module']['pages'] ) ) {
			$reading_module_pages = $module_pages_data['reading_module']['pages'];
		}
		...
		// Check if current page is one of the assigned reading module pages.
		$should_enqueue = false;
		$script_type    = 'index'; // Default script type.

		if ( ! empty( $ielts_pages ) && ! empty( $reading_module_pages ) ) {
			foreach ( $reading_module_pages as $page_key => $page_label ) {
				if ( isset( $ielts_pages[ $page_key ] ) && is_page( $ielts_pages[ $page_key ] ) ) {
					$should_enqueue = true;
					// Set specific script types for practice pages.
					if ( 'reading_passage_practice' === $page_key ) {
						$script_type = 'reading-passage-practice';
					} elseif ( 'result_reading_passage' === $page_key ) {
						$script_type = 'result-reading-passage';
					}
					break;
				}
			}
		...
		// Check for reading passage pages and archives.
		if ( ! $should_enqueue ) {
			if ( is_singular( 'reading-passage' ) ) {
				...
				$should_enqueue = true;
				$script_type    = 'reading-passage-single';
			} elseif ( is_post_type_archive( 'reading-passage' ) || ( is_home() && get_option( 'page_for_posts' ) && get_post_type( get_option( 'page_for_posts' ) ) === 'reading-passage' ) ) {
				// Dequeue styles with handle matching 'elementor-post-*'.
				global $wp_styles;
				if ( isset( $wp_styles->registered ) ) {
					foreach ( $wp_styles->registered as $handle => $style ) {
				...
				$should_enqueue = true;
				$script_type    = 'reading-passage-archive';
			}
		}

		if ( $should_enqueue ) {
			// Define the handle for the script and style based on type.
			$script_handle  = 'ielts-science-reading-' . $script_type;
			$style_handle   = 'ielts-science-reading-' . $script_type . '-css';
			$runtime_handle = 'ielts-science-reading-runtime';
			...
			// Prepare data for localization using reading module pages.
			...
			// Placeholder for reading settings.
			// $setting_instance = new Ieltssci_Reading_Settings();
			// $feed_data        = $setting_instance->reading_types();.
			$feed_data = array(); // Placeholder.
			...
			wp_localize_script( $script_handle, 'ielts_reading_data', $localized_data );
		}
	}
	...
	public function register_custom_rewrite_rules() {
		...
		$uuid_pages = array(
			'result_reading_passage',
			'reading_passage_practice',
		);
	...
	/**
	 * Provide module pages data for the Reading module.
	 *
	 * Adds the Reading module page information to the overall module pages data.
	 *
	 * @param array $module_data Existing module data.
	 * @return array Updated module data with reading module information.
	 */
	public function provide_module_pages_data( $module_data ) {
		$module_data['reading_module'] = array(
			'module_name'   => 'reading_module',
			'section_title' => __( 'Reading Module Pages', 'ielts-science-lms' ),
			'section_desc'  => __( 'Select the pages for the Reading Module.', 'ielts-science-lms' ),
			'pages'         => array(
				'result_reading_passage'   => __( 'Result Reading Passage', 'ielts-science-lms' ),
				'evaluation_history'       => __( 'Evaluation History', 'ielts-science-lms' ),
				'reading_passage_practice' => __( 'Reading Passage Practice', 'ielts-science-lms' ),
			),
		);
