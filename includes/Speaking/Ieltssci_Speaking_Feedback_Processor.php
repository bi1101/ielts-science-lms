...
class Ieltssci_Speaking_Feedback_Processor {
	...
	private function process_pronunciation_assessment_step( $uuid, $attempt = null, $feed, $language = 'en', $source = 'ai', $refetch = '' ) {
		...
		// Get the detailed transcription JSON from the audio attachment's metadata.
		$transcript_json_data = get_post_meta( $audio_id, 'ieltssci_audio_transcription', true );

		if ( empty( $transcript_json_data ) ) {
			throw new Exception( 'No detailed transcription data found for pronunciation assessment. Please transcribe the audio first.' );
		}
		...
		// Make API call.
		$result = $this->api_client->make_pronunciation_assessment_api_call( $audio_path, $transcript_json_data );
